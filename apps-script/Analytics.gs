...
function collectEvents(body) {
  ...
  // NB: GA4 derives Device category / geo from THIS request (Apps Script's
  // server), not the visitor — UrlFetchApp won't let us forward the visitor's
  // User-Agent (it overrides it). The client tags the real device in a custom
  // `device_category` event param instead. See src/lib/analytics.js.
