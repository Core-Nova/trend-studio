...
5. **Sitemap + prerender.** Both come from the single `ROUTES` table in `scripts/seo-routes.mjs` (there is no `public/sitemap.xml`). Every route in `src/App.jsx` needs an entry with a `body` builder and `sources` (the paths whose last commit date becomes `<lastmod>`). Flag routes missing from `ROUTES`, and body builders that hand-write copy instead of reading `src/translations` or `src/data/services.json`.
